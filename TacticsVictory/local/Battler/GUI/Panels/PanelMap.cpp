...
{
    int sizeMapX = Landscape::self->GetSizeX_Columns();
    int sizeMapY = Landscape::self->GetSizeY_Rows();
    ...
    for (int row = 0; row < sizeMapY; row++)
    {
        ...
        {
            float height = Landscape::self->GetHeightAccurately((float)col, (float)row, true);

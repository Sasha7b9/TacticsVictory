...
{
    start.x = (int)_start.x;
    start.y = (int)_start.y;

    finish.x = (int)_finish.x;
    finish.y = (int)_finish.y;
...
void PathFinder::Find(std::function<void(const Array<Integer2D> &path)> funcComplete)
{
    StopSearch();
...
Array<Integer2D> PathFinder::ToArray()
{
    return path;
}
...
void PathFinder::SetSize()
{
    ...
    int sizeX_cols = Landscape::self->GetSizeX_Columns();
    int sizeY_rows = Landscape::self->GetSizeY_Rows();
    ...
    for (int row = 0; row < sizeY_rows; row++)
    {
        ...
        {
            heightMap.At(col, row) = Landscape::self->GetHeightCenter((float)col, (float)row);
        }
    }
...
bool PathFinder::Wave::Contain(const Integer2D &coord)
{
...
{
    points.AddElement(Integer2D{col, row});
    num_wave.At(col, row) = numWave;
}


void PathFinder::AddPrevWave(Array<Integer2D> &path_)
{
    int colX = path_[0].x;
    int rowY = path_[0].y;
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        {
            path_.InsertElement(0, Integer2D(newCol, newRow));
            return;
        }
    }
}
